...
//		executoService = 
//				Executors.newCachedThreadPool(NamedThreadFactory.create("asycn-query-thread-", true));
	...
	public static <T> List<T> invokeAllAndWait(List<Callable<T>> tasks){
		...
		if(CollectionUtils.isNotEmpty(allFutures)){
			List<T> resultList = Lists.newArrayListWithExpectedSize(tasks.size());
